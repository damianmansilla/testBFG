...
(function() {
    ...
    test('test 3 -asn_override', test_handle_request({
        ...
        verify: function(i) {
            ...
            equal(i.response.respond.args[0][0], 'online-1', 'Verifying selected alias');
            equal(i.response.respond.args[0][1], '163.172.15.235', 'Verifying CNAME');
            equal(i.response.setTTL.args[0][0], 20, 'Verifying TTL');
            equal(i.response.setReasonCode.args[0][0], 'D', 'Verifying reason code');
        }
    }));

    test('test 4 -asn_override_fallback', test_handle_request({
        settings: {
            providers: {
                'edgecast': { 'cname': 'wac.6497.edgecastcdn.net', 'padding': 0, 'penal': false },
                'ovh-1': { 'cname': '5.135.137.172', 'padding': 0, 'penal': true, 'surpenal': -15 },
